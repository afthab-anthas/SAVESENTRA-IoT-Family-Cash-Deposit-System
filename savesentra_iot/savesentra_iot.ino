...
void loadUsers() {
  ...
  for (int i = 0; i < userCount; i++) {
    ...
    users[i].role =
        preferences.getString(("role" + String(i)).c_str(), "Other");
  }
  preferences.end();
  Serial.print("Loaded ");
...
void saveUserBalance(int index) {
  ...
  preferences.end();
  Serial.println("Saved balance for " + users[index].name + ": " +
                 String(users[index].balance));
}
...
void updateBlynkUserList() {
  ...
  Blynk.setProperty(V20, "labels", (userCount > 0 ? users[0].name : ""),
...
BLYNK_WRITE(V20) {
  ...
  if (selectedIndex >= 0 && selectedIndex < userCount) {
    ...
    String profileStatus = "Profile: " + users[selectedIndex].name + " (" +
                           users[selectedIndex].role + ")";
    Blynk.virtualWrite(V1, profileStatus);

    // Auto-update the V21 Role Dropdown
    int roleMenuIndex = 3; // Default to Other
    if (users[selectedIndex].role == "Parent")
      roleMenuIndex = 1;
    else if (users[selectedIndex].role == "Child")
      roleMenuIndex = 2;
    Blynk.virtualWrite(V21, roleMenuIndex);

    if (familyGoal > 0) {
      float goalPct =
          (users[selectedIndex].balance / (float)familyGoal) * 100.0;
...
BLYNK_WRITE(V21) {
  if (selectedMenuIndex >= 0 && selectedMenuIndex < userCount) {
    ...
    case 1:
      users[selectedMenuIndex].role = "Parent";
      break;
    case 2:
      users[selectedMenuIndex].role = "Child";
      break;
    case 3:
      users[selectedMenuIndex].role = "Other";
      break;
    default:
      return;
    }
    ...
    preferences.begin("users", false);
    preferences.putString(("role" + String(selectedMenuIndex)).c_str(),
                          users[selectedMenuIndex].role);
    preferences.end();

    String profileStatus = "Profile: " + users[selectedMenuIndex].name + " (" +
                           users[selectedMenuIndex].role + ")";
    Blynk.virtualWrite(V1, profileStatus);
...
BLYNK_WRITE(V8) {
  if (param.asInt() == 1) {
    ...
    if (indexToDelete >= 0 && indexToDelete < userCount) {
      ...
      if (authenticatedUserIndex == indexToDelete)
        authenticatedUserIndex = -1;
      else if (authenticatedUserIndex > indexToDelete)
...
void updateContributionStatus() {
  ...
  if (indexToCalculate != -1 && familyGoal > 0) {
    float goalPercentage =
        (users[indexToCalculate].balance / (float)familyGoal) * 100.0;
    ...
    if (currentIrState == LOW) {
      ...
      if (depositInProgress && timeSinceClear > (motorStopDelay + 3000)) {
        ...
        if (depositAmount > 0.0) {
          String notifyMsg = users[authenticatedUserIndex].name +
                             " deposited " + String(depositAmount, 0) + " AED";
          ...
          String logMessage =
              timestamp + " | " + users[authenticatedUserIndex].name +
              " deposited " + String(depositAmount, 2) + " AED. Total: " +
              String(users[authenticatedUserIndex].balance, 2) +
              " AED. Length Steps: " + String(billLengthSteps);
